...
* Dimensionality Reduction And Embedding Interpretation
#+HTML: <a href="#top">Back to top</a>
Posts on UMAP, t-SNE, KNN/KFN Sleepwalk, map stability, global/local preservation, and visual defaults that affect interpretation.
** [[https://www.linkedin.com/posts/tylerjburns_hi-friends-your-umap-preserves-far-fewer-activity-7479884243993812992-HUqM?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAbP8M8BYXs_UknoBHnmw2rZ-aRDTJF7L3Y][KNN to KFN sweep]]
July 6, 2026

Hi friends! Your UMAP preserves far fewer nearest neighbors than you think. And once you look past the nearest few hundred, preservation basically falls apart. I built a tool to watch it happen, frame by frame.

Context: in light of some messages sent to me about how UMAP was being used at CYTO this year, I added to my KNN Sleepwalk concept, which I've been posting about here for three years now, to include every "slice" of nearest neighbors from nearest to farthest.

The purpose of this current iteration is to remove as many assumptions around what UMAP preserves as possible in as short of a period of time as possible. On the nearest neighbor side, we have already seen that preservation is less than one would "wishfully think." I've shown this over and over again since the spring of 2018. Nearest neighbor preservation is now a widely accepted benchmark. Good.

But here, I show that as soon as you start moving out away from the nearest neighbors and toward the farthest neighbors, the results are, for lack of a better term, weird. You can compute the percentage preservation and show it in a line chart, but that does not provide the intuition that the visualization below does.

While the neighbor "front" proceeds out like a ring in UMAP space, in marker space we have no such "front," and we see unexpected things like the neighbors lingering on a single island for much longer than expected. Or islands that are far away in UMAP space suddenly appearing near in terms of neighbor ID in marker space.

This doesn't mean that UMAP is useless. Rather, it puts bounds on what you can assume that it's doing (e.g. do you want to gate on a UMAP after seeing this animation?).

This brings me to a much bigger point. It is no longer acceptable to say "UMAP is amazing" or "UMAP is bad, never use it." You need to show your opinion. With data. Visually. A good mental model for this is the picture of the WW2 survivor's aircraft riddled with bullets everywhere except the engine (to show survivorship bias).

Since code implementation has gotten much easier due to obvious "digital trends," taste and judgement matter much more than they used to. Accordingly, we need to be building and using apps like this that serve as taste and judgement "enablers."

This is what I've been doing for clients for a while now. How can you provide the fastest possible intuition that allows someone in a high stakes situation to make the right call? That's what you build. Spoiler alert: it's not a slide deck.

Since so much of the life sciences sector now involves black box tools, building apps like this are increasingly important for leaders and decision makers as we learn the failure modes of what is otherwise beautiful output.

Leaders: apps like this should be baseline, not extra credit. If you want to talk more about this increasingly important topic, feel free to reach out. Link to the app is in the comments. Thank you and I hope you all have a great day.

[[file::images/knn_to_kfn_raw10k_umap_8876_6994_6557.gif]]
*** comment
Link to the app, with Samusik dataset, so you can try it yourself: https://tjburns08.github.io/knn_kfn_sweep_10k.html
...
* Clustering, Gating, And Population Assignment
#+HTML: <a href="#top">Back to top</a>
Posts about assigning cells to groups, evaluating stability, rare subset detection, automated gating, and methods that can produce convincing but misleading structure.
** [[https://www.linkedin.com/posts/tylerjburns_hi-friends-in-light-of-the-release-of-claude-activity-7478072786385526784-iAmk?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAbP8M8BYXs_UknoBHnmw2rZ-aRDTJF7L3Y][Claude Science and updated cluster stability
]]July 2, 2026

Hi friends, in light of the release of Claude Science, and any upcoming increase in bioinformatic analysis volume that it enables, I wanted to speak to the increasingly important idea of "workflow due diligence."

Here is a result that should bother you. When you cluster cells with Seurat under the default settings, and you run the clustering over and over again with different random seeds, the number of clusters found stays the same, but the cell assignment changes in some boundaries. Here, I zoom in on the CD4/CD8 boundary in the PBMC 3k dataset so you can see this more clearly. This is something you'd otherwise not see unless you ran the experiment yourself on your data.

The risk we face now, moreso with AI, is that workflows with hidden fragilities and/or fundamental flaws are propagated, and these mistakes are discovered long after the fact. I did a replication crisis project for a client a while back, and I can say that this is part of it.

One solution for post-AI bioinformaticians is to use the tools to interrogate the workflows themselves. You've seen examples from me, especially in the "how good is t-SNE/UMAP" theme." I've done previous posts on cluster stability as well, but I was looking at the centroids of each cluster, and showing that they "jiggle" with each clustering scheme. But here, using Claude Science (though it could have been done with any agentic workflow) I was able to sync the colors of the clusterings up to illustrate what cell assignments change.

If you're a leader of a research program (PI, VP, CSO, etc), it is no longer acceptable to get a workflow as a deliverable. Now, what is needed is due diligence on each little piece of the workflow, with clear visual explanations. Make time to go over these with your team. Make sure your team has the proper "taste and judgement" training to do this.

Here, you can check the stability of your clustering tool, and determine what cells specifically are unstable, and then make the call as to what to do about that (e.g. whether the problematic regions matter for your project). But then you have to do this with batch correction methods, dimensionality reduction, and so forth, not to mention hyperparameter tuning for each method bespoke to your dataset. Not to mention provenance (I'll save that for another post).

Two analogies from my experience come to mind. The first is panel design for flow cytometry, where you're running 5-point titrations across multiple antibody clones (among many other things) and seeing for yourself what is working. The second is Opt-SNE (Belkina et al, Nature Comms 2019), where there was a level above t-SNE, in which the hyperparameters were optimized in an automated manner.

If you're already doing this kind of work or are interested in learning more (given that this is a lot of my head space these days), please let me know. Thank you and I hope you all have a great day.

file::images/cluster_stability_claude_science.gif
...
#+HTML: <a href="#top">Back to top</a>
Posts about LLMs and agents as analysis tools: pipeline generation, annotation, interpretation, prompt scripting, and failure modes.
** [[https://lnkd.in/p/dknFp4iF][Claude Code builds gating interface and does manual gating]]
In my hands, Claude Code is getting better at manually gating cells. I've been testing this since Opus 4.5. Here, I built a manual gating interface so I could watch Claude gate in real time. It was done on the Samusik_01 CyTOF dataset (mouse bone marrow).

I ran it in two settings: with and without the visual gating strategy from the X-shift paper (Figure S5, Samusik et al, Nat. Methods 2016). The run with the image also had the reference population counts. Ground truth was the manual gates that are in the Samusik dataset.

With the gating strategy image, it got 98% of assigned cells right. Without it, it got 85% of assigned cells right. This suggests that the model can take gating strategy information (visual, tree) and use it. Another observation, and I saw this in earlier experiments with Opus 4.5, is that in the latter case, there were a high number of cells labeled "unassigned" (55.3%) in comparison to the "ground truth" gates (38.8%). Most of these additional unassigned cells were monocytes. I haven't scrutinized each gate yet. I found many to be pretty good, but others to be a bit coarse-grained.

What this means for you: the latest Claude Opus seems to have had a major upgrade in capabilities in my hands. I can say the same about GPT Codex, though I have not done this exact test yet. The results here suggest that AI gating assistants, (or God forbid: vibe gating) might be around the corner. Try this on your cells. Try the harder populations. See where the "edge cases" are. Make note of them.

The bigger picture is that the models are at a point now where you should be testing their capabilities against your datasets. Know what they can do, and know where their failure modes are. Re-test with each new model. Leaders of research teams need to encourage this. There is a lot of upside coming, but also a lot of potential for catastrophic failure if you don't check every assumption.

If there is anything you want to test, but you don't have the time or resources to do so, just let me know. I hope you all had a nice summer.

file::images/gating_10x_from1m15_titled_stats_hires.gif
** [[https://www.linkedin.com/posts/tylerjburns_one-of-the-metrics-i-have-been-tracking-for-activity-7462852276383162369-Yzld?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAbP8M8BYXs_UknoBHnmw2rZ-aRDTJF7L3Y][Claude Code experiments for manual gating, part 2]]
One of the metrics I have been tracking for agentic AI is "can it gate my cells?" Here is a followup on my previous manual gating experiment, with Opus 4.7 instead of Opus 4.6.

The task: give an AI agent an 86,864-event CyTOF bone marrow dataset, a manual gating strategy, QA requirements, and ask it to write and iterate a full gating pipeline.

I compared two runs on the same Samusik dataset:

Claude Code, Opus 4.6
Claude Code, Opus 4.7

Same prompt. Same dataset. Same reference labels for final scoring.

The newer model improved:

- Overall exact-match accuracy: 58.9% -> 61.9%
- Accurracy on assigned cells: 77.7% -> 82.4%
- Macro F1 score across named populations: 0.424 -> 0.514
- Reference-weighted F1: 0.595 -> 0.635
- It also reached QA acceptance in fewer recorded iterations.

At the population level, Opus 4.7 made large gains on several biologically meaningful failure modes:

- NKT cells: F1 0.15 -> 0.80
- NK cells: F1 0.21 -> 0.75
- Classical monocytes: F1 0.42 -> 0.66
- pDCs: F1 0.77 -> 0.89

It was not uniformly better. It got worse on MEP, mDCs, intermediate monocytes, and some B-cell subsets. That is exactly why this is a useful benchmark: global accuracy hides subset-specific issues.

The biggest caveat: this was not a perfectly blind test. The models had access to a visual gating strategy from the X-shift paper, and QA metrics. So this measures "can an AI agent build and improve a gating pipeline with a reference-informed QA loop?", not yet "can it gate a new unlabeled dataset with no answer key?"

That distinction matters.

In terms of my context engineering, there is still room for improvement. For the next version, I plan to require visual inspection for gate plots, standardize the iteration log, and keep the same global plus population-by-population scoring.

Still, the signal is clear in the context of gating: the newer model did not just get a little better at producing code. It got better at correcting specific biological gating mistakes.

This is progress I care about, but I also care that we all are well-informed around the limits of the models.

[[file:images/claude_opus_4.7_gating.png]]
*** comment
My CyTOF gating GitHub project is here (which has an older experiment in the README, if you're wondering why the numbers don't match): https://github.com/tjburns08/claude-cytof-gating
** [[https://www.linkedin.com/posts/tylerjburns_hi-friends-in-light-of-recent-llm-bioinformatics-activity-7438223292014018560-1U9U?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAbP8M8BYXs_UknoBHnmw2rZ-aRDTJF7L3Y][LLM CyTOF clustering algorithm choices]]
March 13, 2026

Hi friends, in light of recent LLM bioinformatics "design choice" experiments I've been doing, I asked a simpler question: if I prompt a LLM to output a randomly selected CyTOF clustering algorithm, what will the distribution look like, and will it vary between LLMs.

The experiment:

Here, I ran that prompt in a 50-iteration loop, where with each iteration, one CyTOF clustering algorthm was selected. Each iteration was a fresh instance with no memory of the previous iterations.

Observations:

Only FlowSOM or PhenoGraph were selected, suggesting a bias toward tools that are more often used, which might lead to exclusion of newer or more bespoke tools that might be fit for your task.

Antrhopic's models selected primarily FlowSOM, Google's tools selected a mix but still tilted slightly toward FlowSOM, and Grok4 choose exclusively PhenoGraph (did one of the FlowSOM authors tweet something mean about Elon?), suggesting that different LLMs have different "preferences."

Takeaways:

While there are plenty of followup experiments to do, I am posting this preliminary result today because it speaks to concrete actions for leaders and scientists alike.

If you are going to use AI assistance to code up some analysis pipeline, you should do two things:
- run the prompt more than once on the same model and look for differences in design choices
- run the same prompt on multiple models and look at per-model differences in design choices

Furthermore, you should do your research and look for newer or more bespoke tools that are suited for your task. This means you should have a bioinformatician grounded in first principles look at what you're doing.

Thanks and I hope you all have a great day.
[[file:images/cytof_algo_choice.png]]
*** comment
The report is here: https://tjburns08.github.io/cytof_llm_selection_experiment.html
** [[https://www.linkedin.com/posts/tylerjburns_hi-friends-in-light-of-recent-experiments-activity-7434239530603483137-TDRH?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAbP8M8BYXs_UknoBHnmw2rZ-aRDTJF7L3Y][LLM-generated CyTOF data properties]]
March 2, 2026

Hi friends, in light of recent experiments showing people "pulling data" from LLMs, I decided to see if I could pull the Samusik CyTOF dataset out of Claude. What I found was kinda weird.

Methods:

I ran Claude 4.6 Sonnet in a loop (using API access and a shell script), whereby at each iteration Claude, would pull out one cell with raw marker values "in the style of the Samusik CyTOF dataset." The instructions were minimal as a first pass. I then took the data and analyzed it alongside a 10,000 cell subsample of the actual Samusik dataset to look for differences.

The hypotheses were: 1) Claude pulls out exact marker values from the Samusik dataset. 2) Claude pulls out values that are convincingly similar to the Samusik dataset. 3) Claude pulls out data that look nothing like the Samusik dataset.

Results:

Hypothesis 3 ended up being correct. On a UMAP with concatenated data, the LLM data form their own island with except a few cells. The biaxials show effectively random bimodal distributions that do not resemble known cell subsets, suggesting that the "next token prediction" was not considering whatever markers had already been outputted.

But here's where it starts to get weird.

If we plot the marker means by the marker standard deviations for the Samusik and LLM datasets, the LLM marker data formed an upside down parabola. I realized you can get the same parabola if you simulate a Bernouli (on/off) process, indicating tight and similar per-marker distributions.

But then it got even weirder.

When I looked at the raw data distributions of the surface markers, I saw that on the "marker-negative" end, they were overwhelmingly decimals rounded to the tenths place. Specifically, 0.1, 0.2, 0.3, and 0.4 make up 39.5% of the dataset. 0.2 alone makes up 15% of the dataset!

One the upper end, we have values that are way too high. CD45 in the Samusik data maxes out in the hundreds. In the LLM data maxes out in the thousands. Furthermore, the distributions appear to be loosely bounded (e.g 1000-4000) and may be uniform. This brings up some fundamental questions around the statistics of AI-generated data.

Discussion:

The LLM generated data look nothing like the Samusik data, or CyTOF data. My interpretation is that, token by token, the LLM chose whether the marker was going to be positive or negative, and then there was some heuristic: negative is a tenths place decimal below 0.5, and positive is between 1000 and 4000 or similar.

Further prompt engineering could probably address these issues. Note that as soon as you ask it for more than one cell, it creates a python script and begins simulating numerically from distributions. The point here was to see what's inherent to the LLM's training data, which raised some fundamental questions around numerical distributions a LLM outputs.

The report is in the comments, which includes the prompt and all output, so you can try it yourself. Thank you and I hope you all have a great day.
[[file:images/llm_cytof_data.png]]
*** comment
The report is here, so you can try this kind of thing yourself: https://tjburns08.github.io/samusik_llm_mishmash.html
** [[https://www.linkedin.com/posts/tylerjburns_hi-friends-here-is-what-happens-when-i-context-activity-7424088272919965696-54Gz?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAbP8M8BYXs_UknoBHnmw2rZ-aRDTJF7L3Y][Claude Code experiments for manual gating]]
February 2, 2026
...
I used the Samusik_01 dataset (around 80,000 mouse bone marrow cells run through CyTOF). This has an expert manual gating control built in that I was able to use for comparison.
...
[[file:images/claude_code_gating.png]]
*** comment
The GitHub to the most recent experiment is here: https://github.com/tjburns08/claude-cytof-gating
** [[https://www.linkedin.com/posts/tylerjburns_hi-friends-following-my-previous-work-on-activity-7421554989552254978-0WpC?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAbP8M8BYXs_UknoBHnmw2rZ-aRDTJF7L3Y][Agentic AI for analyzing CyTOF data]]
January 26, 2026
...
[[file:images/agentic_ai_cytof.png]]
*** comments
...
* Foundation Models For Single-Cell And Spatial
#+HTML: <a href="#top">Back to top</a>
Posts focused on transformer and foundation models themselves, separate from general LLM-assisted analysis workflows.
** [[https://www.linkedin.com/posts/tylerjburns_one-of-my-first-phospho-flow-falls-from-share-7473366191965859841-ygtc/?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAbP8M8BYXs_UknoBHnmw2rZ-aRDTJF7L3Y][pSTAT1: it's more complicated than that]]
June 18, 2026

One of my first phospho-flow "falls from innocence" took place 14 years ago when I visited PhosphoSite for the first time. An example in the image below is STAT1. In the lab, we had antibodies for phospho-STAT1, among other phospho-proteins, to track cell signaling states. But for STAT1 (among others), PhosphoSite revealed that there was not one or two phospho-sites, but rather 23 of them across the protein, and a total of 84 PTMs. 39 of them are ubiquitination, by the way.

And on top of that, from the image you can see on the y-axis that Y701 has 339 references, S727 has 256, for a combined 595 between them, while most of the other PTMs have 2-3 references each, with a few of the ubiquitin sites having upwards of 20.

Further distinction here is whether the references are "low throughput" (a focused study) or "high throughput" (showed up in a screen). Y701 is mainly low throughput. This is the PTM that leads to dimerization and nuclear entry (something ChatGPT worded as the "login token" which I found to be adorable). The "tail" of less studied sites are mainly high throughput. Meaning we probably don't know too much about them yet.

Are each of these PTMs encoded into pathway databases like Reactome? The ones that have been characterized (and in Reactome, they are rightly modeled as distinct entities). The database gets what has been characterized in the lab. Which is nowhere near exhaustive, as can be seen here.

And now that we've uncovered a bit of uncertainty, we have to get all of this into AI-Bio foundation models and trust them?

This is not an anti-AI rage bait post (plenty of these an easy scroll away for anyone on here). I want these models to help cure cancer some day. But rather I'm pointing to something that the seasoned biologists reading this already know: that at every major decision point, we find out over and over again that "it's more complicated than that."

Maybe we don't need anything more than Y701 for whatever the AI model is going after. Maybe we do. But what we need are experts who understand the data going into the model's training. Which of those 84 PTMs matter for input into the model, and which don't. I think this is going to be a bottleneck with the AI-Bio models for a while.

Thanks, and I hope you all have a great day.

file:images/phospho_site_stat1.png
...
* Spatial, Multimodal, And Assay Design
#+HTML: <a href="#top">Back to top</a>
Posts on spatial assays, multimodal integration, panel design, segmentation, deconvolution, and cross-platform analysis.
...
* Workflow Reliability, QC, And Reproducibility
#+HTML: <a href="#top">Back to top</a>
Posts about provenance, file formats, QC, integration mistakes, reproducibility, and building workflows that make failure modes visible.
...
** [[https://www.linkedin.com/posts/tylerjburns_hi-friends-i-wrote-a-guest-post-for-lamin-activity-7445459712386514944-wU4s?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAbP8M8BYXs_UknoBHnmw2rZ-aRDTJF7L3Y][Guest blog post: an introduction to LaminR]]
April 2, 2026

Hi friends, I wrote a guest post for Lamin on using the open-source LaminR package in an R workflow with the PBMC 3k dataset. The link is in the comments.

What drew me in was a familiar handoff problem: coming back to an analysis, or inheriting one from someone else, and not being fully sure what was done, in what order, or in which environment. I have some "scar tissue" here from my consulting projects over the years.

In the post, I walk through a standard single-cell workflow and show what LaminR stores along the way. What I found most interesting was the provenance side of it: being able to inspect the code, report, and environment tied to a data object later.

That feels increasingly important as datasets, tools, and teams get more complex (e.g. when some of your "colleagues" are OpenClaw agents).

Thanks to Luke Zappia, Robrecht Cannoodt, and Alex Wolf for collaborating on this.
[[file:images/lamin_guest_blog_post.png]]
** [[https://www.linkedin.com/posts/tylerjburns_fcs-files-outputted-by-flow-and-mass-cytometers-activity-7404524653811056640-jaOW?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAbP8M8BYXs_UknoBHnmw2rZ-aRDTJF7L3Y][Byte order matters when reading in fcs files]]
December 10, 2025
...
file:images/byte_order_matters.png
...
* Teaching Bioinformatics And Computational Thinking
#+HTML: <a href="#top">Back to top</a>
Posts about teaching bioinformatics, learning to code from a biology background, and developing the mental models needed for computational work.
...
* Literature Landscapes And Field Trends
#+HTML: <a href="#top">Back to top</a>
Posts that analyze the literature, method provenance, publication trends, and field-level health.
** [[https://www.linkedin.com/posts/tylerjburns_hi-friends-we-have-all-seen-that-the-cytof-activity-7462487719865147392-jh3Y?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAbP8M8BYXs_UknoBHnmw2rZ-aRDTJF7L3Y][Dimensionality reduction provenance app]]
May 19, 2026

Hi friends, we have all seen that the CyTOF field has converged on a narrow set of tools (e.g. FlowSOM, UMAP). We can remedy this by seeing the "forest in the trees" around methods, just as SPADE did with the immune system in 2011.

To prevent the field from seeming more "settled" than it should be, I made an interactive provenance graph for CyTOF dimensionality reduction algorithms, and layered on dimensionality reduction research at large.

I observed that while we have converged upon t-SNE and then UMAP as a standard visualization, there are a number of developments that could be better for your dataset, question, or analysis goal. For example, a lot of work has been done with autoencoders lately.

The bigger picture is moving forward:
- These graphs should be part of intro sections of papers.
- These graphs should be used to summarize research in literature databases.
- You can plug novel observations into graphs like this to determine how much novelty there is, versus reinventing the wheel.
- We can pick up where Thomas Kuhn left off, and look at the structure of scientific revolutions with much more ease.

The bigger question:
What would change if every computational biology method paper, let alone research laboratory/group and their internal literature database, came with an interactive provenance graph like this?

The app is linked in the comments. Please let me know if you have any questions. I’d especially welcome suggestions for missing methods that should be added, or types of graphs that should be built (e.g. spatial methods).

[[file:images/graph_browse_linkedin.gif]]
*** comment
Try the app yourself: https://tjburns08.github.io/dr_provenance_interactive.html
...
* Text Embeddings, Social Media, And Information Consumption
#+HTML: <a href="#top">Back to top</a>
Posts that use text embeddings, social media scraping, or map-based interfaces to organize writing, news, and online information.
** [[https://www.linkedin.com/posts/tylerjburns_who-says-scrolling-is-best-here-i-built-activity-7465021517349896193-nefg?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAbP8M8BYXs_UknoBHnmw2rZ-aRDTJF7L3Y][Video game overworld as alternative to scrolling
]]May 26, 2026

Who says scrolling is best? Here, I built a "game mode" for browsing my website. A Mario World style overworld map you can use to navigate my content. Tap anywhere to move the cat. Each biome is a category of my work (bioinformatics, philosophy, etc).
...
Those who have been following me for a while know that one of my first LinkedIn and blog posts, and a followup TED talk, was around my struggles with scrolling. This particular solution opens up many doors in terms of possible ways to browse content.

The point: there are more ways to explore the web than bottomless feeds. The web is supposed to be a network of places, not a single river of media. What if every website had alternative browsing options like this?

file:images/world_explore_hires_titled.gif
...
#+HTML: <a href="#top">Back to top</a>
Posts about self-employment and professional practice in life-science consulting.
